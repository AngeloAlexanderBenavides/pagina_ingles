...
import { curriculum, Phase, Unit, Exercise, libraryTopics, storeItems } from "@/lib/data"
...
const buildPhaseExamQuestions = (phase: Phase): Exercise[] => {
  const pool = phase.units.flatMap(unit => unit.exercises || [])
  const fallback: Exercise[] = [
    {
      id: `${phase.id}-fallback-1`,
      question: "Select the correct form of the verb 'to be'",
      options: ["I am", "I are", "I is", "I be"],
      correctAnswer: 0,
      explanation: "Use 'am' with the pronoun 'I'."
    },
    {
      id: `${phase.id}-fallback-2`,
    ...
    {
      id: `${phase.id}-fallback-3`,
      question: "Pick the right preposition: The cat is ___ the table.",
      options: ["on", "in", "at", "by"],
      correctAnswer: 0,
      explanation: "Use 'on' to describe something resting on a surface."
    },
    {
      id: `${phase.id}-fallback-4`,
  ...
  const poolToUse = pool.length > 0 ? pool : fallback
  return shuffleArray(poolToUse).slice(0, Math.min(EXAM_QUESTION_TARGET, poolToUse.length))
}

const getPhaseExamSize = (phase: Phase) => {
  const poolLength = phase.units.flatMap(unit => unit.exercises || []).length
  if (poolLength === 0) {
    return Math.min(EXAM_QUESTION_TARGET, 4)
  }
  return Math.min(EXAM_QUESTION_TARGET, poolLength)
}
...
const LevelNode = ({
  ...
  const phaseColors = [
    { main: "#00E0FF", shadow: "#0099b0" },
    { main: "#7B61FF", shadow: "#5a43cc" },
    { main: "#10B981", shadow: "#059669" },
    { main: "#F59E0B", shadow: "#D97706" },
    { main: "#F43F5E", shadow: "#E11D48" },
  ]
...
export default function StudentDashboard() {
  ...
  const [selectedExamPhase, setSelectedExamPhase] = useState<Phase | null>(null)
  ...
  const getPhaseExamId = (phaseId: string) => `${phaseId}-exam`

  const handleStartExam = (phase: Phase) => {
    const questions = buildPhaseExamQuestions(phase)
    setExamQuestions(questions)
    setSelectedExamPhase(phase)
  }

  const handleExamComplete = (score: number) => {
    if (!selectedExamPhase) return

    const examId = getPhaseExamId(selectedExamPhase.id)
    updateProgress(examId, score)
    setCompletedUnits(prev => prev.includes(examId) ? prev : [...prev, examId])
    setSelectedExamPhase(null)

    const totalQuestions = examQuestions.length || getPhaseExamSize(selectedExamPhase)
    const passScore = Math.ceil(totalQuestions * EXAM_PASS_RATE)
    const passed = score >= passScore

    toast({
      title: passed ? "¡Examen aprobado!" : "Resultado registrado",
      description: passed
        ? `Puntaje: ${score}/${totalQuestions}. Avanza a la siguiente fase.`
        : `Puntaje: ${score}/${totalQuestions}. Puedes reintentar cuando quieras.`
    })
  }
  ...
  return (
    <div className="flex h-screen bg-[#050A18] text-white overflow-hidden font-sans">
      <AnimatePresence>
        ...
        {selectedExamPhase && examQuestions.length > 0 && (
          <ExamView
            phaseTitle={selectedExamPhase.title}
            questions={examQuestions}
            onClose={() => setSelectedExamPhase(null)}
      ...
      <main className="flex-1 relative overflow-y-auto bg-[#050A18]">
        ...
        <div className="max-w-[600px] mx-auto pb-32 px-4 relative z-10">
          {activeTab === "ruta" && (
            <div className="w-full mb-20 relative">
              {curriculum.map((phase, phaseIndex) => {
                const phaseUnitsCompleted = phase.units.every(unit => completedUnits.includes(unit.id))
                const examId = getPhaseExamId(phase.id)
                // @ts-ignore
                const examScore = user?.progress?.[examId]
                const examSize = getPhaseExamSize(phase)
                const examPassed = examScore !== undefined && examScore >= Math.ceil(examSize * EXAM_PASS_RATE)
                ...
                return (
                  ...
                  >
                    {/* Phase Header */}
                    <motion.div className="text-center mb-16 relative z-20" variants={fadeUp}>
                      <div className={cn(
                      ...
                      <h2 className="text-2xl md:text-3xl font-extrabold">{phase.title}</h2>
                      <p className="text-sm md:text-base text-[#8899A6] mt-2">{phase.level}</p>
                    </motion.div>

                    {/* SVG Path Background for this Phase */}
                    <motion.div
                      className="relative w-full"
                      style={{ height: `${phase.units.length * 160 + (phaseIndex < curriculum.length - 1 ? 80 : 0)}px` }}
                      variants={scaleIn}
                    >
                          ...
                          d={`M${(0 + phaseIndex) % 3 === 1 ? 120 : (0 + phaseIndex) % 3 === 2 ? 280 : 200},80 ${phase.units.map((_, i) => {
                            ...
                            if (i === phase.units.length - 1) {
                              if (phaseIndex < curriculum.length - 1) {
                                ...
                                const nextPhaseStartIndex = phaseIndex + 1
                    ...
                    {phaseIndex < curriculum.length - 1 && (
                      <motion.div
                        className="mt-12 bg-[#0B1121] border-2 border-[#1F2937] rounded-2xl p-6 w-full max-w-md mx-auto relative z-20"
                        variants={scaleIn}
                      >
                        ...
                        <div className="flex items-center justify-between gap-3 text-sm text-[#8899A6] flex-wrap">
                          <div className="flex items-center gap-2">
                            <Trophy className="w-4 h-4" />
                            <span>
                              {phaseUnitsCompleted
                                ? "Examen de fase disponible"
                                : "Completa todas las unidades para habilitar el examen"}
                            </span>
                          </div>
                          <Button
                            variant="outline"
                            className={cn(
                              "border-[#374151] text-white hover:bg-[#1F2937]",
                              phaseUnitsCompleted ? "" : "opacity-60 cursor-not-allowed"
                            )}
                            disabled={!phaseUnitsCompleted}
                            onClick={() => handleStartExam(phase)}
                          >
                            {examScore === undefined && phaseUnitsCompleted && "Rendir examen"}
                            {examScore === undefined && !phaseUnitsCompleted && "Bloqueado"}
                            {examScore !== undefined && !examPassed && `Reintentar (${examScore}/${examSize})`}
                            {examScore !== undefined && examPassed && `Aprobado (${examScore}/${examSize})`}
                          </Button>
                        </div>
                      </motion.div>
                    )}
          ...
          {activeTab === "tutor" && (
            <motion.div
              className="flex flex-col items-center justify-center h-[600px] text-center max-w-2xl mx-auto"
              initial="hidden"
              animate="visible"
              variants={fadeUp}
            >
              <motion.div className="w-24 h-24 bg-[#00E0FF]/10 rounded-full flex items-center justify-center mb-8 border border-[#00E0FF]/30" variants={scaleIn}>
                <Brain className="w-12 h-12 text-[#00E0FF]" />
              </motion.div>
              <motion.h2 className="text-2xl md:text-4xl font-extrabold mb-4 text-white" variants={fadeUp}>AI Tutor Personal</motion.h2>
              <motion.p className="text-[#8899A6] text-base md:text-lg mb-8" variants={fadeUp}>
              ...
              <motion.div className="bg-[#111827] border border-[#1F2937] rounded-2xl p-6 w-full text-left mb-8" variants={scaleIn}>
              ...
              <motion.div variants={fadeUp}>
                <Button className="bg-[#00E0FF] text-[#050A18] hover:bg-[#00E0FF]/90 font-bold px-8 py-6 text-base md:text-lg rounded-xl">
          ...
          {activeTab === "speaking" && (
            <motion.div
              ...
              variants={fadeUp}
            >
              <motion.div className="w-24 h-24 bg-[#F43F5E]/10 rounded-full flex items-center justify-center mb-8 border border-[#F43F5E]/30" variants={scaleIn}>
                <Mic className="w-12 h-12 text-[#F43F5E]" />
              </motion.div>
              <motion.h2 className="text-2xl md:text-4xl font-extrabold mb-4 text-white" variants={fadeUp}>Speaking Lab</motion.h2>
              <motion.p className="text-[#8899A6] text-base md:text-lg mb-8" variants={fadeUp}>
                Mejora tu pronunciación con análisis de voz en tiempo real. Lee las frases y obtén una puntuación de precisión.
              </motion.p>

              <motion.div className="bg-[#111827] border border-[#1F2937] rounded-2xl p-8 w-full mb-8 relative overflow-hidden" variants={scaleIn}>
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-[#F43F5E] to-transparent opacity-50" />
                <p className="text-xl md:text-2xl font-serif italic text-white mb-8">"The quick brown fox jumps over the lazy dog."</p>
              ...
              <motion.div variants={fadeUp}>
                <Button className="bg-[#F43F5E] text-white hover:bg-[#F43F5E]/90 font-bold px-8 py-6 text-base md:text-lg rounded-xl flex items-center gap-3">
          ...
          {activeTab === "library" && (
            <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
              <div className="col-span-full mb-4">
                <h2 className="text-2xl md:text-3xl font-extrabold text-white">Librería de Temas</h2>
                <p className="text-sm md:text-base text-[#8899A6]">Explora recursos adicionales por categoría.</p>
              </div>
          ...
          {activeTab === "ranking" && (
            <motion.div className="max-w-2xl mx-auto" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
              <div className="text-center mb-10">
                <h2 className="text-2xl md:text-3xl font-extrabold text-white mb-2">Tabla de Clasificación</h2>
                <p className="text-sm md:text-base text-[#8899A6]">Compite con otros estudiantes y sube de liga.</p>
              </div>

              <motion.div className="bg-[#111827] border border-[#1F2937] rounded-2xl overflow-hidden" variants={scaleIn}>
          ...
          {activeTab === "store" && (
            ...
            >
              <motion.div className="text-center mb-10" variants={fadeUp} transition={{ delay: 0.15 }}>
                ...
                <h2 className="text-2xl md:text-3xl font-extrabold text-white mb-2">Tienda de Items</h2>
                <p className="text-sm md:text-base text-[#8899A6]">Canjea tus gemas por potenciadores y vidas extra.</p>
              </motion.div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {storeItems.map((item) => {
                  ...
                  return (
                    <motion.div
                      key={item.id}
                      className="bg-[#111827] border border-[#1F2937] p-6 rounded-2xl flex flex-col relative overflow-hidden group hover:border-[#00E0FF]/50 transition-all"
                      variants={scaleIn}
                      initial="hidden"
                      animate="visible"
